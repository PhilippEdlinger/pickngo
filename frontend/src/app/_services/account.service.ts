...
export class AccountService {
  ...
  login(username, password) {
    ...
    return this.http.post<UserDetails>(`${environment.apiUrl}/users/login`, { username, password })
      .pipe(map(user => {
  ...
  register(user: UserDetails) {
    console.log(user);
    return this.http.post<UserDetails>(`${environment.apiUrl}/users/register`, user)
      .pipe(map(savedUser => {
        // store user details and jwt token in local storage to keep user logged in between page refreshes
